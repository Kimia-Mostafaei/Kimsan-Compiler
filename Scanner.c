...
/* Note: put \n as case '\n'*/
		...
		case '.':
			...
			return currentToken;
		case '\'':
			currentToken.code = QOUT_T;
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		case '_':
			currentToken.code = UNDER_T;
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		
		/* Cases for arithemetic operator*/
		case '+':
		...
		case '>':
			...
			return currentToken;
		case '=<':
			currentToken.code = OP_LTHEN;
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		case '>=':
			currentToken.code = OP_GTHEN;
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		case '++':
			currentToken.code = OP_INC;
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		/*Cases for logical operator*/
		case 'and':
...
int nextState(int state, kimsan_bytes c) {
	int col;
	int next;
	col = nextClass(c);
...
/*
	[A-z],[0-9], #,   \n,   .,   \",    _,      (	   other
	 L(0), D(1), C(2), N(3), P(4),Q1(5), U(6), B1(7)   O(8)
*/
int nextClass(kimsan_bytes c) {
	int val = -1;
	switch (c) {
	case CHRCOL2:
		val = 2;
...
Token funcIL(str lexeme) {      // shouldnt also check for float litterals 
	Token currentToken = { 0 };
	kimsan_long tlong;
...
Token funcID(str lexeme) {
	...
	kimsan_bytes lastch = lexeme[length - 1];
	int isID = False;
	switch (lastch) {
		case RPR_T: 
...
Token funcKEY(str lexeme) {
	...
	else { // else not error it returns a variable
...
kimsan_void printToken(Token t) {
