 ...
 * -------------------------------------------------------------
 */
kimsan_void bErrorPrint(str fmt, ...);
kimsan_void displayBuffer(Buffer* ptr_Buffer);
kimsan_long getFileSize(str fname);
int isNumber(const str ns);
kimsan_void startReader(str, str, kimsan_bytes, int, int);
...
************************************************************
*/
kimsan_void startReader(str program, str input, kimsan_bytes mode, int size, int increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	int loadSize = 0;		/* the size of the file loaded in the buffer */
	kimsan_bytes symbol;			/* symbol read from input file */
