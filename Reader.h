...
typedef struct bufferReader {
	...
	int		size;				/* current dynamic memory size (in kimsan_byte) allocated to character buffer */
	int		increment;			/* character array increment factor */
	int		mode;				/* operational mode indicator */
	kimsan_byte		flags;				/* contains character array reallocation flag and end-of-buffer flag */
...
BufferPointer	readerAddChar		(BufferPointer const, kimsan_bytes);
kimsan_boln		readerClear		    (BufferPointer const);
kimsan_boln		readerFree		    (BufferPointer const);
kimsan_boln		readerIsFull		(BufferPointer const);
kimsan_boln		readerIsEmpty		(BufferPointer const);
kimsan_boln		readerSetMark		(BufferPointer const, int);
int		readerPrint		    (BufferPointer const);
int		readerLoad			(BufferPointer const, FILE* const);
kimsan_boln		readerRecover		(BufferPointer const);
kimsan_boln		readerRetract		(BufferPointer const);
kimsan_boln		readerRestore		(BufferPointer const);
/* Getters */
kimsan_bytes		readerGetChar		(BufferPointer const);
...
kimsan_byte		readerGetFlags		(BufferPointer const);
kimsan_void		readerPrintStat		(BufferPointer const);
