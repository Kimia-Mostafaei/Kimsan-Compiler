...
extern kimsan_void startParser(kimsan_void);
extern int startScanner(BufferPointer sc_buf);

static kimsan_void printParserError(str fmt, ...);
static kimsan_void displayParser(BufferPointer ptrBuffer);
static kimsan_long getParserFileSize(str fname);
static kimsan_void callGarbageCollector(kimsan_void);
...
kimsan_void printParserError(str fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(kimsan_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
kimsan_long getParserFileSize(str fname) {
	FILE* input;
	kimsan_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
kimsan_void displayParser(BufferPointer ptrBuffer) {
...
kimsan_void callGarbageCollector(kimsan_void) {
